...
var (
	...
	lastFrameTime = time.Now() // entonces, timePerFrame = (time.Second / 60).Seconds()
)

var cursorRotation = 0.0

// UpdateDeltaTime actualiza el reloj interno para calculo de tiempo delta entre frames
func UpdateDeltaTime() {
	deltaTime = time.Since(lastFrameTime).Seconds()
	lastFrameTime = time.Now()
}

// DrawMessages dibuja los mensajes informativos en la escena principal
...
// AnimateCursor realiza la animacion de balanceo del cursor segun el pitch de la musica
...
// UpdateBackColor cambia Blue y Red del color de fondo segun la posicion del cursor
...
// UpdateMusicPitch interpolacion lineal del pitch de la musica segun la posicion del cursor
